...
function makeState(overrides: Partial<RootState> = {}): RootState {
  return {
    ...
    selection: { names: [] },
    config: { status: 'loaded', config: { startDate: null, defaultName: null }, error: null },
...
describe('App', () => {
  ...
  it('renders the config error banner when the config fetch failed', () => {
    const store = createStore(
      rootReducer,
      makeState({
        config: { status: 'error', config: { startDate: null, defaultName: null }, error: 'Config fetch failed' },
      }),
    );
    render(
      <Provider store={store}>
