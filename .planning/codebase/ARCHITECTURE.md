...
- **Shared types are built artifacts:** `shared` is consumed from `dist/` (`main: dist/index.js`), so any edit to `shared/src/types.ts` requires `pnpm --filter @planning-espoir/shared run build` before api/webapp typecheck or tests (all root scripts build it first).
